...
export class OrgsPage implements OnInit {
  ...
  private pageSize = 6;
  private scrollStates = new Map<string, { page: number; displayed: Organization[] }>();
    ...
    programs: [
      {
        ...
        description: 'Classical Aikido training following O-Sensei\'s original teachings',
        level: 'All Levels',
        duration: 'Ongoing',
      ...
      },
      {
        ...
        description: 'Comprehensive teacher training program for qualified practitioners',
      ...
      },
      {
        ...
        description: 'Character-building martial arts program for children and teens',
        level: 'Ages 6-17',
    ...
    lineageFeatures: [
      {
        ...
        description: 'Authentic techniques preserved from O-Sensei\'s original teachings'
      },
      {
        icon: 'ribbon',
        title: 'International Recognition',
        description: 'Ranks and certifications recognized by Aikikai Foundation'
      },
      {
        icon: 'star',
  ...
  ngOnInit() {
    ...
    this.organizationsService.organizations$.subscribe(organizations => {
      console.log('[Orgs Page] Received organizations from service:', organizations.length);
  ...
  get filteredOrganizations(): Organization[] {
    ...
    if (this.filterMode === 'single' && this.filteredEntityId) {
      filtered = this.organizations.filter(org => org.id === this.filteredEntityId);
  ...
  private updateDisplayedOrganizations() {
    ...
    if (filterChanged || needsReload) {
      ...
      console.log('[Orgs Page] Displayed organizations:', this.displayedOrganizations.length);
    }
  }

  private loadInitialOrganizations(state: { page: number; displayed: Organization[] }, source: Organization[]) {
    state.page = 0;
    state.displayed = [];
    this.loadMoreOrganizationsForState(state, source);
  }

  private loadMoreOrganizationsForState(state: { page: number; displayed: Organization[] }, source: Organization[]) {
