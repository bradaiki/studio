...
export class ChatInvitationNotificationComponent implements OnInit {
  ...
  async acceptInvitation() {
    if (this.isProcessing || this.isExpired || this.invitation.status !== 'pending') {
      return;
    }
    ...
    try {
      ...
      await this.showSuccessToast(`Joined ${this.chatName || 'chat'} successfully!`);
  ...
  canActOnInvitation(): boolean {
    return this.invitation.status === 'pending' && !this.isExpired && !this.isProcessing;
  }
