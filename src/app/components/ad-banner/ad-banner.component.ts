    ...
    <div class="ad-container" *ngIf="showAds" [ngClass]="'ad-' + format">
      ...
      <div class="ad-slot" #adContainer>
        ...
        <ins *ngIf="isProduction && hasValidConfig"
          ...
          [attr.data-ad-format]="format === 'banner' ? 'horizontal' : 'auto'"
          [attr.data-full-width-responsive]="'true'">
        </ins>
        <!-- Placeholder shown in development or when AdSense isn't configured -->
        <div *ngIf="!isProduction || !hasValidConfig" class="ad-placeholder">
          <div class="placeholder-icon">📢</div>
          <div class="placeholder-text">Ad Space</div>
          <div class="placeholder-subtext">{{ format === 'banner' ? '320×50 Banner' : '300×250 Rectangle' }}</div>
        </div>
      </div>
    </div>
  `,
...
export class AdBannerComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.hasValidConfig = !!(this.adClient && this.adSlotId
